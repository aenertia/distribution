...
# Shader support (options: none, scanline, lcd, sharp)
SHADER=$(get_setting "drastic_shader" "${PLATFORM}" "${GAME}")
if [ -z "${SHADER}" ] || [ "${SHADER}" = "default" ]; then
    SHADER="none"
fi
export DSHOOK_SHADER="${SHADER}"
