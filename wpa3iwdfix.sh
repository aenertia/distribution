#!/bin/bash
# Applies Crypto API config changes to linux.aarch64.conf for any Rockchip device.
...
if [ -z "$1" ]; then
    echo "Usage: $0 <path_to_linux.aarch64.conf>"
    echo "Example: $0 projects/ROCKNIX/devices/RK3566/linux/linux.aarch64.conf"
    exit 1
fi
...
#    (If it is already 'm', sed will safely ignore this step).
