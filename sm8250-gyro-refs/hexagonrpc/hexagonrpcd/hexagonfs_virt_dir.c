...
/*
 * Context for virtual directory file descriptors.
 * Tracks the dirent list and current position for readdir.
 */
struct virt_dir_ctx {
	const struct hexagonfs_dirent *const *list;
	size_t pos;
};
...
{
	struct virt_dir_ctx *dirctx = dir->data;
	const struct hexagonfs_dirent *ent;
	struct hexagonfs_fd *fd;
	int ret;

	ent = walk_dir(dirctx->list, segment);
	if (ent == NULL)
		return -ENOENT;
