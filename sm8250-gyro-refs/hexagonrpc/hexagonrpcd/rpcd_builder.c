...
struct hexagonfs_dirent *construct_root_dir(const char *prefix, const char *dsp)
{
	...
	sns_reg_ver = malloc(n_prefix + strlen(SNS_REG_VERSION) + 1);
	...
	return hfs_mkdir("/", 7,
			...
			vendor_dir,
			hfs_map_or_empty("sns_reg_version", sns_reg_ver)
		);
}
